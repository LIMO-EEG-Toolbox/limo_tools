...
elseif strcmp(defaults.analysis,'Frequency') 
    ...
    if isempty(defaults.lowf)
        LIMO.data.start = EEG.etc.tf_freqs(1);
        LIMO.data.trim1 = 1;
    elseif defaults.lowf < EEG.etc.tf_freqs(1)
        ...
        LIMO.data.start = EEG.etc.tf_freqs(position);
        LIMO.data.trim1 = position; 
    end
    
    % end
    if isempty(defaults.highf)
        LIMO.data.end = EEG.etc.tf_freqs(end);
        LIMO.data.trim2 = length(EEG.etc.tf_freqs);
    elseif defaults.highf > EEG.etc.tf_freqs(end)
        ...
        LIMO.data.end = EEG.etc.tf_freqs(position);
        LIMO.data.trim2 = position; 
    end
    
    LIMO.data.freqlist = EEG.etc.tf_freqs(LIMO.data.trim1:LIMO.data.trim2);
...
elseif strcmp(defaults.analysis,'Time-Frequency') || strcmp(defaults.analysis,'ITC')
    ...
    if isempty(defaults.highf)
        ...
        LIMO.data.trim_high_f = length(EEG.etc.tf_freqs);
    elseif defaults.highf > EEG.etc.tf_freqs(end)
        error(['The highest frequency possible is:',num2str(EEG.etc.tf_freqs(end))]);
        return
    else
