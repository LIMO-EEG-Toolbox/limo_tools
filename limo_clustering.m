...
%
% INPUT
% M     = 2D matrix of observed statistic values (for a single channel use
%         1 in the 1st dimension)
% P     = 2D matrix of observed p values (for a single channel use 1 in the
%         1st dimension)
...
% p     = Threshold. Used both to form the clusters as well as the critical
%         p value on the bootstrapped statistic distribution
% fig   = 1/0 to plot the maximum stat under H0 (if empty, turned on if no 
%         significant statistics are found)
%
% OUTPUT
% mask      = A labelled matrix of the same size as M corresponding to a
%             threshold p corrected for multiple comparisons
% cluster_p = The p-values obtained via the matrix bootM (corrected)
% max_th    = Cluster mass threshold controlling the type 1 FWER
%
% Cyril Pernet 
...
if MCC == 2 && size(bootM,1)>1
    ...
    parfor boot = 1:nboot
        ...
        [clusterslabelmat, nclusters] = limo_findcluster(bootP(:,:,boot) <= p, channeighbstructmat, minnbchan);
