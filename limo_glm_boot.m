    ...
    case 'IRLS'
        ...
        parfor B = 1:nboot
            [BETASB{B},MODELR2{B}, MODELF{B},MODELp{B},...
                ...
                F_CONTVALUES{B},p_CONTVALUES{B}]=glm_iterate(centered_y,X,Weights,boot_table(:,B))
        end
end
...
%% ----------------------------------------------------------------------------------------
function [Betas,Rsquare, F_Rsquare,p_Rsquare,F_conditions, pval_conditions, F_interactions, pval_interactions, F_continuous,pval_continuous]=glm_iterate(centered_y,X,Weights,boot_table)
...
for frame = size(centered_y,2):-1:1
    Y     = centered_y(boot_table,frame); % resample Y
    W     = Weights(boot_table,frame);    % apply same sampling as Y
    WY    = Y .* repmat(W,1,size(Y,2));
    WX    = X .* repmat(W,1,size(X,2));
    Betas = pinv(WX)*WY;
